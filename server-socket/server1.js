...
function Getnotification(playerID, NotificationCount, callback) {
    ...
    DBconn.query('SELECT notification FROM notifications where playerid =? limit ' + NotificationCount, playerID, function (err , records) {
...
function insertNoti (_id , noti  , callback){
	var record = {playerid : _id ,notification : noti  }; 
	 DBconn.query('insert into   notifications  set ? ',record , function (err , rows) {
        if (err) { throw err; }
           callback(rows);
...
function insertReco (_id , Reco  , callback){
	var record = {playerid : _id ,list : Reco  }; 
	 DBconn.query('insert into   recommendeded   set ? ',record , function (err , rows) {
        if (err) { throw err; }
           callback(rows);
...
function GetRecommendation(playerID , callback) {
    
    DBconn.query('SELECT list  FROM recommendeded where playerid   =? ', playerID, function (err , records) {
        if (err) console.log(err);
        if ( records.length > 0){try {
        callback(records[0]['list']);
...
socket.on('pushNoti', function(data){
	_id = data['id']; 
	noti = data['noti' ];
insertlog ( _id , noti , function(res){
console.log(res); 
}) ; 
...
socket.on('pushRecommendation', function(data){
	_id = data['id']; 
	reco = data['reco' ];
	console.log("preco"+id);
insertReco ( _id , reco , function(res){
console.log(res); 
}) ; 

}); 
socket.on('recomendation' , function(data){
 	  	var par = data['parameters'];
 	var playerid = par['playerid'];
 	GetRecommendation(playerid ,  function(records){
io.sockets.emit('recomendationResponse' , {description : 'recomendation retrieved ' , recomendation : records});
 	});
 });
 ...
 /*process_login('lala' , '312' , function (res){
            res['description'] = 'login info'; 
 console.log(reseedewewwrewr); });*/
 

http.listen(5000 ,'0.0.0.0' , function(){
  console.log('listening on :5000');
  //PostMewBroadcast('114' , 'ewrre' ,'oiregjreoipgjreoijgre' , function(a,b){ console.log(b);}  ) ;
//insertNoti(114, 'some one attacked you check the game' , function (res){ console.log(res); }); 
//Getnotification('114' ,5, function(res){console.log(res); }); 
// insertReco(1, '1,2,3,4,5,6' , function (res ) { console.log(res) ; } ); 
//GetRecommendation(1 ,  function(records){ console.log(records); }) ; 
GetPasswordOfCertainMail( 'mona.adel20151995@gmail.com' , function(res) { console.log(res) ;}) ; 
